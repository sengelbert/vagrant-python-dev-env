...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision :shell, path: "fedora-install.sh", privileged: false
  ...
  config.vm.provider "virtualbox" do |v|
    ...
    v.customize ['modifyvm', :id, '--clipboard-mode', 'bidirectional'] # Copy/Paste
    v.customize ['modifyvm', :id, '--draganddrop', 'bidirectional'] # Drag & Drop
  end
